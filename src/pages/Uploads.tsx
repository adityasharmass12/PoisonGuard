  ...
  try {
    ...
    rows.filter((_, i) => i % sampleStep === 0).forEach((row, si) => {
      ...
      if (urlVal && seen.has(urlVal)) {
        anomalies.push({ row: actualIdx + 1, reason: `Duplicate value: ${urlVal.slice(0, 60)}`, severity: 'medium' });
        suspicious++;
      } else if (urlVal) {
        seen.add(urlVal);
      }
...
interface UploadProps {
  onUploadComplete?: (data: any, file: string) => void;
}
      ...
      <AnimatePresence mode="wait">
        ...
        {uploadState === 'done' && result && (
          <motion.div key="done" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            ...
            {/* Pre-check summary */}
            {result.preCheckStats && result.preCheckStats.total > 0 && (
